...
#include "sounds/Looking_for_updates.h"
...
void setup() {
  ...
  if (digitalRead(BTN_PLAY_STOP) == LOW) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      PLAY_SOUND(No_Wifi_update_skipped);
    }
    flashAllLEDs(2);
  } else {
...
void loadSdWifi() {
  ...
  if (sep <= 0) {
    ...
    PLAY_SOUND(Bad_format_in_ssid_txt);
    return;
  }
...
void checkOTA() {
  ...
  PLAY_SOUND(Looking_for_new_version);
  HTTPClient http;
...
void downloadAudioFiles() {
  ...
  if (newTracks > 0) {
    PLAY_SOUND(New_media_downloaded);
  } else {
    PLAY_SOUND(No_new_media_to_download);
  }
}
